...
## Overview
This repository contains files necessary to generate ASP for the OFS F-series devkit
...
* `ofs_fseries-dk_usm`: ASP that supports shared virtual memory between host and device. This 
variant is the same as the non-USM variant with the addition of the USM path between 
the kernel-system and the host.

* `ofs_fseries-dk`:  DMA-based ASP that supports local memory and host memory interfaces for the 
kernel system.
